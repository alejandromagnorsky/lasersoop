...
public class LevelLoader {
	...
	/**
	 * Crea una nueva instancia de LevelLoader con el nombre de un archivo de
	 * nivel asociado.
	 * 
	 * @param filename
	 ...
	 * 
	 * @return Devuelve una instancia de TileSet con los datos levantados
	 *         cargados. 'null' si hubo algún error.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Parser que recorre una dada línea buscando una cantidad dada de enteros,
	 * de acuerdo con el formato prefijado para los archivos de nivel.
	 * 
	 * @param line
	 *            String del cual el parser extraerá los datos.
	 * @param lineType
	 *            Cantidad de enteros a levantar. - Para una línea con
	 *            información acerca de la dimensión del tablero, lineType es
	 *            igual a 2, ya que son 2 los enteros que tiene que levantar (un
	 *            para la cantidad de filas y otro para la de columnas). - Para
	 *            una línea con información acerca de una celda, lineType es
	 *            igual a 7.
	 * @return Devuelve un arreglo con los enteros extraidos, en orden de
	 *         aparición. 'null' si la línea no cumple con el formato prefijado
	 *         para los archivos de nivel.
	 */
		...
		for (int i = 0; i < line.length() && !flagComment; i++) {
			if ((auxChar = line.charAt(i)) != ' ' && auxChar != '\t') {
				if (auxChar == ',' && quantComas < lineType - 1
					 ...
					 * coma no sea vacío. O sea, evita líneas como estas: -
					 ...
					 * levantado no sea vacío. O sea, evita líneas como estas: -
	...
	/**
	 * Dada una línea, devuelve todo lo que esté antes de '#' en caso de
	 * haberlo.
	 * 
	 * @param line
	 *            Línea a leer.
	...
	/**
	 * Levanta las dimensiones del tablero. Valida que estén en el rango de
	 * valores aceptados.
	 * 
	 * @param line
	 *            Línea a leer. Se espera que dicha linea sea tal que contenga
	 *            información sobre las dimensiones. Sino se considera inválida.
	 * @return Arreglo de 2 enteros, una para la cantidad de filas y otra para
	 *         la de columnas. 'null' sino levanta o no valida.
	 */
	public Integer[] loadDimensions(String line) {
		Integer[] dim = parser(line, DIMLINE);
		if (dim == null || dim[0] < 5 || dim[1] < 5 || dim[1] > 20
				|| dim[0] > 20) {
			System.out
					.println("TEMP|-----| fil o cols mayores a 20 o menores a 5 o coso null");
			return null;
		}
		return dim;
	}
	...
	/**
	 * Levanta la información acerca de una celda. Valida que los valores sean
	 * coherentes entre si y con el tablero de juego.
	 * 
	 * @param line
	 *            Línea a leer. Se espera que dicha linea sea tal que contenga
	 *            información sobre una celda. Sino se considera inválida.
	 * @param tileSet
	 *            Tablero al cual pertence dicha celda.
	 * @return Arreglo de 7 enteros. 'null' si no levanta o no valida.
	 */
	public Integer[] loadTile(String line, TileSet tileSet) {
		...
		if (tile == null) {
			...
			/* Valido los parámetros del color */
				...
				|| tile[4] > 255 || tile[5] < 0 || tile[5] > 255 || tile[6] < 0 || tile[6] > 255))
	...
	public static boolean isInLevels(File file) {
		...
		File levelsDir = (new File("levels")).getAbsoluteFile();
		boolean flag = false;
		File absFile = file.getAbsoluteFile();

		if (absFile.getParent().equals(levelsDir.getPath()))
			flag = true;
